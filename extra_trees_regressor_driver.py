...
parameters from paper: 'bootstrap': False,
                       'ccp_alpha': 0.0,
...
def gen_etr_model( x: pd.DataFrame, y: pd.DataFrame, hyperparam_tuning=False, params={}):
    """Generates the Extra Trees Regressor model.
    ...
    if hyperparam_tuning:
        ...
        model_parameter = ExtraTreesRegressor()
    ...
    model_gen = ExtraTreesRegressor(bootstrap=params['bootstrap'],
    ...
    model_save(model_gen, name='ETR.pkl')
    return model_gen

# Load model if it exists
model = model_load('ETR.pkl')
...
if model is None:
    params_base = {'bootstrap': False,
                   ...
                   'min_samples_leaf': 1,
                   'min_samples_split': 2,}
    model = gen_etr_model(X_train, y_train, hyperparam_tuning=False, params=params_base)
...
plt.title("ETR Actual vs Predicted Values")
