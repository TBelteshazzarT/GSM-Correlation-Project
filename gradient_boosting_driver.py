...
parameters from paper: 'n_estimators': 200,
                       'ccp_alpha': 0.001,
...
def gen_gb_model( x: pd.DataFrame, y: pd.DataFrame, hyperparam_tuning=False, params={}):
    """Generates the Gradient Boosting Regressor model.
    ...
    if hyperparam_tuning:
        ...
        model_parameter = GradientBoostingRegressor()
    ...
    model_gen = GradientBoostingRegressor(n_estimators=params['n_estimators'],
    ...
    model_save(model_gen, name='GB.pkl')
    return model_gen

# Load model if it exists
model = model_load('GB.pkl')
...
if model is None:
    params_base = {'n_estimators': 200,
                   ...
                   'min_samples_leaf': 8,
                   'min_samples_split': 16}
    model = gen_gb_model(X_train, y_train, hyperparam_tuning=False, params=params_base)
...
plt.title("GB Actual vs Predicted Values")
